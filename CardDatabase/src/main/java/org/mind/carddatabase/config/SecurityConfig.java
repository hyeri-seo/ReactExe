...
직접 서버의 주소로 접근하려고 하면 무조건 login 화면으로 이동한다.
user 사용자

Console 창에 아래와 같이 출력됨. 개발자가 사용하는 비밀번호
Using generated security password: 27420206-4796-4a68-bb34-7a7bb9f43690

스프링 시큐리티
1) 권한 부여(ROLE 부여)
    해당 서비스 접근 가능 여부
    인증(Authentication): 당신이 누구인지 증명해라, id/password
    인가(Authrization): 권한이 있으면 허용해줄게, ROLE(역할)
2) PASSWORD 암호화
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable()
                ...
                // 다른 요청을 인증 과정을 거쳐야 접근할 수 있음
                .anyRequest().authenticated().and()
                // 잘못된 인증 정보 요청 시, 오류 응답 처리를 authEntryPoint가 함
                .exceptionHandling()
                .authenticationEntryPoint(authEntryPoint).and()
                // /login을 제외한 나머지 모든 요청을 필터를 통과해야 정상 응답을 받을 수 있음
